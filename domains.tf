...
resource "godaddy_domain_purchase" "main" {
  domain       = each.key
  years_leased = 0
  enable_privacy = true
  auto_renew     = true
  nameservers    = aws_route53_zone.main[each.key].name_servers

  admin {
    email      = local.domain_registrant.email
    first_name = local.domain_registrant.first_name
    last_name  = local.domain_registrant.last_name
    phone      = local.domain_registrant.phone_number

    address {
      line_1      = local.domain_registrant.address_line_1
      city        = local.domain_registrant.city
      country     = local.domain_registrant.country_code
      postal_code = local.domain_registrant.zip_code
      #state       = local.domain_registrant.state
  ...
  for_each = { for domain_name, domain in var.domains : domain_name => domain.domain_registration if domain.domain_registration.registrar == "godaddy" }
}
